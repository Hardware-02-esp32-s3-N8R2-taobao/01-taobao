...
- 应用固件大小：`954.1 KB`
- 应用分区大小：`4032.0 KB`
- 应用分区剩余：`3077.9 KB`，约 `76%`
- Bootloader 大小：`20.1 KB`
- Bootloader 剩余：`11.9 KB`，约 `37%`
- Flash 实际使用：`875.1 KB`
- 镜像总大小：`954.0 KB`
- IRAM 使用：`98.8 KB`，剩余 `215.0 KB`，约 `31.5%`
- 已初始化数据 `.data`：`10.6 KB`
- 未初始化数据 `.bss`：`19.8 KB`
- RAM 中可执行代码 `.text`：`68.3 KB`
- Flash 中代码 `.text`：`722.9 KB`
- Flash 中常量 `.rodata`：`150.0 KB`

## 核心资源可视化

- 应用分区占用：`954.1 KB` / `4032.0 KB`  [######------------------] 23.7%
- Bootloader 区占用：`20.1 KB` / `32.0 KB`  [###############---------] 63.0%
- IRAM 占用：`98.8 KB` / `313.8 KB`  [########----------------] 31.5%
- RAM 静态占用(.data + .bss)：`30.4 KB`
...
- 当前 IRAM 总量（按本次链接结果可见）：`313.8 KB`，其中已使用 `98.8 KB`。
- 当前 RAM 静态占用（`.data + .bss`）：`30.4 KB`，这部分会在程序运行时常驻内存。
...
- 应用固件大小：`977,024 B`
- 应用分区大小：`4,128,768 B`
- 应用分区剩余：`3,151,744 B`，约 `76%`
- Bootloader 大小：`20,592 B`
- Bootloader 剩余：`12,176 B`，约 `37%`
- Flash 实际使用：`896,062 B`
- 镜像总大小：`976,910 B`
- IRAM 使用：`101,136 B`，剩余 `220,160 B`，约 `31.5%`
- 已初始化数据 `.data`：`10,892 B`
- 未初始化数据 `.bss`：`20,288 B`
- RAM 中可执行代码 `.text`：`69,956 B`
- Flash 中代码 `.text`：`740,206 B`
- Flash 中常量 `.rodata`：`153,584 B`
...
- 当前用户固件在该分区中的实际占比：`954.1 KB` / `4032.0 KB`，约 `23.7%`。

## 最大空间占用模块

- `libmbedcrypto.a`：`129.7 KB`（`132,813 B`）
- `liblwip.a`：`127.6 KB`（`130,689 B`）
- `libnet80211.a`：`127.0 KB`（`130,063 B`）
