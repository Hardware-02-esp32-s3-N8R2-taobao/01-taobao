...
- Flash 中代码 `.text`：`755.5 KB`
- Flash 中常量 `.rodata`：`158.3 KB`

## 核心资源可视化

- 应用分区占用：`997.2 KB` / `4032.0 KB`  [######------------------] 24.7%
...
- 应用固件大小：`1,021,120 B`
- 应用分区大小：`4,128,768 B`
- 应用分区剩余：`3,107,648 B`，约 `75%`
- Bootloader 大小：`20,592 B`
- Bootloader 剩余：`12,176 B`，约 `37%`
- Flash 实际使用：`938,076 B`
- 镜像总大小：`1,020,994 B`
- IRAM 使用：`106,094 B`，剩余 `215,202 B`，约 `33.0%`
- 已初始化数据 `.data`：`10,908 B`
- 未初始化数据 `.bss`：`23,176 B`
- RAM 中可执行代码 `.text`：`72,010 B`
- Flash 中代码 `.text`：`773,676 B`
- Flash 中常量 `.rodata`：`162,128 B`
...
- 当前用户固件在该分区中的实际占比：`997.2 KB` / `4032.0 KB`，约 `24.7%`。
