...
- 应用固件大小：`996.3 KB`
- 应用分区大小：`4032.0 KB`
- 应用分区剩余：`3035.7 KB`，约 `75%`
- Bootloader 大小：`20.1 KB`
- Bootloader 剩余：`11.9 KB`，约 `37%`
- Flash 实际使用：`915.2 KB`
- 镜像总大小：`996.2 KB`
- IRAM 使用：`103.6 KB`，剩余 `210.2 KB`，约 `33.0%`
- 已初始化数据 `.data`：`10.7 KB`
- 未初始化数据 `.bss`：`22.6 KB`
- RAM 中可执行代码 `.text`：`70.3 KB`
- Flash 中代码 `.text`：`754.8 KB`
- Flash 中常量 `.rodata`：`158.2 KB`

## 核心资源可视化

- 应用分区占用：`996.3 KB` / `4032.0 KB`  [######------------------] 24.7%
- Bootloader 区占用：`20.1 KB` / `32.0 KB`  [###############---------] 63.0%
- IRAM 占用：`103.6 KB` / `313.8 KB`  [########----------------] 33.0%
- RAM 静态占用(.data + .bss)：`33.3 KB`
...
- 当前 IRAM 总量（按本次链接结果可见）：`313.8 KB`，其中已使用 `103.6 KB`。
- 当前 RAM 静态占用（`.data + .bss`）：`33.3 KB`，这部分会在程序运行时常驻内存。
...
- 应用固件大小：`1,020,192 B`
- 应用分区大小：`4,128,768 B`
- 应用分区剩余：`3,108,576 B`，约 `75%`
- Bootloader 大小：`20,592 B`
- Bootloader 剩余：`12,176 B`，约 `37%`
- Flash 实际使用：`937,146 B`
- 镜像总大小：`1,020,064 B`
- IRAM 使用：`106,086 B`，剩余 `215,210 B`，约 `33.0%`
- 已初始化数据 `.data`：`10,908 B`
- 未初始化数据 `.bss`：`23,168 B`
- RAM 中可执行代码 `.text`：`72,010 B`
- Flash 中代码 `.text`：`772,922 B`
- Flash 中常量 `.rodata`：`161,952 B`
...
- 当前用户固件在该分区中的实际占比：`996.3 KB` / `4032.0 KB`，约 `24.7%`。
