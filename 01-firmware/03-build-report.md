...
- 应用固件大小：`1009.1 KB`
- 应用分区大小：`4032.0 KB`
- 应用分区剩余：`3022.9 KB`，约 `75%`
- Bootloader 大小：`20.1 KB`
- Bootloader 剩余：`11.9 KB`，约 `37%`
- Flash 实际使用：`924.4 KB`
- 镜像总大小：`1009.0 KB`
- IRAM 使用：`107.2 KB`，剩余 `206.6 KB`，约 `34.2%`
- 已初始化数据 `.data`：`10.7 KB`
- 未初始化数据 `.bss`：`22.6 KB`
- RAM 中可执行代码 `.text`：`73.8 KB`
- Flash 中代码 `.text`：`761.8 KB`
- Flash 中常量 `.rodata`：`160.4 KB`

## 核心资源可视化

- 应用分区占用：`1009.1 KB` / `4032.0 KB`  [######------------------] 25.0%
- Bootloader 区占用：`20.1 KB` / `32.0 KB`  [###############---------] 63.0%
- IRAM 占用：`107.2 KB` / `313.8 KB`  [########----------------] 34.2%
- RAM 静态占用(.data + .bss)：`33.4 KB`
...
- 当前 IRAM 总量（按本次链接结果可见）：`313.8 KB`，其中已使用 `107.2 KB`。
- 当前 RAM 静态占用（`.data + .bss`）：`33.4 KB`，这部分会在程序运行时常驻内存。
...
- 应用固件大小：`1,033,328 B`
- 应用分区大小：`4,128,768 B`
- 应用分区剩余：`3,095,440 B`，约 `75%`
- Bootloader 大小：`20,592 B`
- Bootloader 剩余：`12,176 B`，约 `37%`
- Flash 实际使用：`946,602 B`
- 镜像总大小：`1,033,174 B`
- IRAM 使用：`109,756 B`，剩余 `211,540 B`，约 `34.2%`
- 已初始化数据 `.data`：`11,004 B`
- 未初始化数据 `.bss`：`23,184 B`
- RAM 中可执行代码 `.text`：`75,568 B`
- Flash 中代码 `.text`：`780,130 B`
- Flash 中常量 `.rodata`：`164,200 B`
...
- 当前用户固件在该分区中的实际占比：`1009.1 KB` / `4032.0 KB`，约 `25.0%`。

## 最大空间占用模块

- `libnet80211.a`：`131.3 KB`（`134,408 B`）
- `libmbedcrypto.a`：`129.7 KB`（`132,809 B`）
- `liblwip.a`：`127.6 KB`（`130,689 B`）
- `libc.a`：`80.2 KB`（`82,151 B`）
- `libwpa_supplicant.a`：`75.2 KB`（`77,035 B`）
- `libpp.a`：`52.8 KB`（`54,049 B`）
- `libmain.a`：`41.2 KB`（`42,195 B`）
- `libesp_hw_support.a`：`35.4 KB`（`36,224 B`）
