...
- 应用固件大小：`958.2 KB`
- 应用分区大小：`4032.0 KB`
- 应用分区剩余：`3073.8 KB`，约 `76%`
- Bootloader 大小：`20.1 KB`
- Bootloader 剩余：`11.9 KB`，约 `37%`
- Flash 实际使用：`879.2 KB`
- 镜像总大小：`958.1 KB`
- IRAM 使用：`98.9 KB`，剩余 `214.9 KB`，约 `31.5%`
- 已初始化数据 `.data`：`10.6 KB`
- 未初始化数据 `.bss`：`19.9 KB`
- RAM 中可执行代码 `.text`：`68.3 KB`
- Flash 中代码 `.text`：`726.4 KB`
- Flash 中常量 `.rodata`：`150.6 KB`

## 核心资源可视化

- 应用分区占用：`958.2 KB` / `4032.0 KB`  [######------------------] 23.8%
- Bootloader 区占用：`20.1 KB` / `32.0 KB`  [###############---------] 63.0%
- IRAM 占用：`98.9 KB` / `313.8 KB`  [########----------------] 31.5%
- RAM 静态占用(.data + .bss)：`30.5 KB`
...
- 当前 IRAM 总量（按本次链接结果可见）：`313.8 KB`，其中已使用 `98.9 KB`。
- 当前 RAM 静态占用（`.data + .bss`）：`30.5 KB`，这部分会在程序运行时常驻内存。
...
- 应用固件大小：`981,232 B`
- 应用分区大小：`4,128,768 B`
- 应用分区剩余：`3,147,536 B`，约 `76%`
- Bootloader 大小：`20,592 B`
- Bootloader 剩余：`12,176 B`，约 `37%`
- Flash 实际使用：`900,274 B`
- 镜像总大小：`981,122 B`
- IRAM 使用：`101,232 B`，剩余 `220,064 B`，约 `31.5%`
- 已初始化数据 `.data`：`10,892 B`
- 未初始化数据 `.bss`：`20,384 B`
- RAM 中可执行代码 `.text`：`69,956 B`
- Flash 中代码 `.text`：`743,794 B`
- Flash 中常量 `.rodata`：`154,208 B`
...
- 当前用户固件在该分区中的实际占比：`958.2 KB` / `4032.0 KB`，约 `23.8%`。

## 最大空间占用模块

- `libnet80211.a`：`129.9 KB`（`133,054 B`）
